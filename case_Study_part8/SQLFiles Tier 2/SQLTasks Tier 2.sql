...
Ans : Tennis Court 1,Tennis Court 2, Massage Room 1, Massage Room 2, Squash Court
...
SELECT CONCAT_WS(' ',info.firstname, info.surname) as fullname, Facilities.name
FROM Facilities
INNER JOIN (
            SELECT Members.firstname, Members.surname, Bookings.facid
            FROM Members
            INNER JOIN Bookings USING ( memid )) AS info
USING ( facid )
...
WHERE date( starttime ) = '2012-09-14' AND ((Facilities.membercost * Bookings.slots) >30 OR (Facilities.guestcost * Bookings.slots) >30)
...
WHERE Bookings.bookid in (SELECT bookid 
      FROM Bookings 
